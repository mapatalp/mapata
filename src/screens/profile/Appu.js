...
function Appu({ onToggleDarkMode, dark }) {
  ...
  const [iconPosition, setIconPosition] = React.useState("leading");
...
const Label = (props) => {
  const theme = useTheme();
  return (
    <Text style={[styles.label, { ...theme.fonts.medium }]}>
      {props.children}
    </Text>
  );
};
...
export default function AppWithProviders() {
  ...
  return (
    ...
    >
      <SafeAreaProvider>
        <Appu onToggleDarkMode={onToggleDarkMode} dark={dark} />
      </SafeAreaProvider>
    </PaperProvider>
  );
}
...
const TwitterFollowButton = (props) => {
  return (
    <Button
      ...
      icon="twitter"
      style={styles.twitterButton}
      onPress={() => Linking.openURL(`https://twitter.com/USERNAME1`)}
    >
      USERNAME1
    </Button>
  );
